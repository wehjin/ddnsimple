...
fn fetch_ip_response() -> Result<Response, AppError> {
    reqwest::get("https://api.ipify.org").map_err(|error| AppError::NoResponseFromIpService(error.description().to_owned()))
}

fn read_text_from_response(mut response: Response) -> Result<String, AppError> {
    response.text().map_err(|error| AppError::NoTextInIpServiceResponse(error.description().to_owned()))
}
